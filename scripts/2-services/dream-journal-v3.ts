    ...
    return {
      ...
      async fetch(req) {
        ...
        if (url.pathname === '/health') {
          return Response.json({ status: 'dreaming', version: '3.0' });
        }
        ...
        if (url.pathname === '/dreams') {
          return Response.json(journalInstance.handleGetDreams());
        }
        
        // Get patterns
        if (url.pathname === '/patterns') {
          return Response.json(journalInstance.handleGetPatterns());
        }
        ...
        if (url.pathname === '/analytics') {
          return Response.json(journalInstance.getDreamAnalytics());
        }
