    ...
    return {
      ...
      fetch: async (req) => {
        ...
        if (url.pathname === '/health') {
          return Response.json({ status: 'ok', service: 'user-profile-service', port: 8904 });
        }
        ...
        if (userMatch && req.method === 'PATCH') {
          try {
            ...
            if (!updated) {
              return Response.json({ success: false, error: 'User not found or no changes' }, { status: 404 });
            }
            const user = this.db.getUser(userId);
            return Response.json({ success: true, user });
          } catch (error: any) {
        ...
        if (interactionMatch && req.method === 'GET') {
          ...
          return Response.json({ success: true, interactions, count: interactions.length });
        }
