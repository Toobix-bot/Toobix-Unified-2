...
class UserProfileService {
  ...
  serve(): Serve {
    return {
      port: 8937 /* Changed from 8904 to avoid conflict with Value Crisis */,
      fetch: async (req) => {
        ...
        if (url.pathname === '/health') {
          return this.jsonWithCors({ status: 'ok', service: 'user-profile-service', port: 8937 /* Changed from 8904 to avoid conflict with Value Crisis */ });
        }
