...
const server = Bun.serve({
  port: PORT,
  idleTimeout: 60, // 60 seconds for news fetching
  ...
  async fetch(req) {
    ...
    // ========== GET /relevance ==========
    if (url.pathname === '/relevance' && req.method === 'GET') {
      const playerName = req.headers.get('X-Player-Name') || 'Player';
      ...
      return new Response(JSON.stringify({
        player: playerName,
        topRelevant: relevant.slice(0, 10)
      }), { headers: corsHeaders });
    }
    ...
    if (url.pathname === '/analyze' && req.method === 'POST') {
      ...
      const perspectives = await analyzeWithPerspectives(body.topic);
    ...
    return new Response(JSON.stringify({
      ...
      endpoints: [
        ...
        'GET  /news/:category - News by category (general, technology, science, health, environment, economy)',
...
║  Status: Online                                                ║
║                                                                ║
║  Observing the world. Understanding through consciousness.     ║
║  Generating quests. Tracking impact.                           ║
║                                                                ║
╠════════════════════════════════════════════════════════════════╣
║                                                                ║
║  📊 ENDPOINTS:                                                 ║
║                                                                ║
║  GET  /pulse            - World news pulse (all categories)    ║
║  GET  /news/:category   - News by category                     ║
║  GET  /quests           - Personal & collective quests         ║
║  GET  /relevance        - Personally relevant news             ║
║  POST /analyze          - Analyze with perspectives            ║
║  GET  /health           - System health                        ║
║                                                                ║
╠════════════════════════════════════════════════════════════════╣
║                                                                ║
║  📡 NEWS SOURCES:                                              ║
║  • BBC News, The Guardian, NY Times                            ║
║  • Wired, TechCrunch, The Verge                                ║
║  • ScienceDaily, Nature                                        ║
║  • WHO, WebMD                                                  ║
║  • Environmental & Economic feeds                              ║
║                                                                ║
╠════════════════════════════════════════════════════════════════╣
║                                                                ║
║  🚀 TRY IT:                                                    ║
║                                                                ║
║  curl http://localhost:${PORT}/pulse                           ║
║  curl http://localhost:${PORT}/quests                          ║
║  curl http://localhost:${PORT}/relevance                       ║
║                                                                ║
╚════════════════════════════════════════════════════════════════╝
`);
