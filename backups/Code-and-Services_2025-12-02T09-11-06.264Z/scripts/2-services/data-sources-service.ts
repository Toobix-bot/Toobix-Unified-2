      ...
      fetch: async (req) => {
        ...
        if (url.pathname === '/weather' && req.method === 'POST') {
          try {
            ...
            if (!body.location) {
              return this.jsonWithCors({ success: false, error: 'Missing required field: location' }, 400);
            }
        ...
        if (url.pathname === '/wikipedia' && req.method === 'POST') {
          try {
            ...
            if (!body.query) {
              return this.jsonWithCors({ success: false, error: 'Missing required field: query' }, 400);
            }
        ...
        if (url.pathname === '/cache' && req.method === 'DELETE') {
          this.cache.clear();
          return this.jsonWithCors({ success: true, message: 'Cache cleared' });
        }
